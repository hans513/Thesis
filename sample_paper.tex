...
Solutions to Roothaan equation are called molecular orbitals, are represented in our basis as $\sum_i c_{ia}\phi_a$, and can be used to get density matrix $\rho = CC^*$ (or 2 $CC^*$ depending on which convention you want to use for $C_{occ}$) by using the assumption of 2 electrons per molecular orbital starting from the lowest-energy one.
