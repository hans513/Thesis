...
However, since wave functions cannot represent classical uncertainty, density matrices are the exactly and only way to solve the problem. Molecular orbitals are orbitals created when two or more elements interact and are usually described using a linear combination of atomic orbitals. Density matrix is a probability matrix of the molecular orbitals within the system and is given by
...
$\epsilon$ is a diagonal matrix of the orbital energies. $C$ is the coefficient matrix of all the states. In RHF it can be used to get the density matrix $\rho$ =  $C_{occ}{C_{occ}}^*$ in which $C_{occ}$ is the coefficient matrix of all the occupied states.
...
\]
This matrix will reduce to the identity  if we have an orthonormal basis set. 
