...
HF uses a linear combination of orbital wave-functions to define the wave-function of the system. The algorithm uses the density operator represented as a density matrix, derived from the wave-functions. The density matrix is a probability matrix of the molecular orbitals within the system, which is given by
\[
\rho = \sum_i\sum_\sigma|\phi_i(\sigma)|^2
\]


Where $\phi_i$ is the molecular orbital dependent on $\sigma$, the spin coordinate. In Restricted Hartree-Fock (RHF) which is most commonly used model of HF, which assumes that all the orbitals are fully occupied. Given the Pauli exclusion principle each orbital will carry two electrons. Thus the equations is formulated to the following:

\[
\rho = 2\sum_i|\phi_i|^2
\]

With the density matrix, the Hamiltonian can be approximated to the Fock operator as shown below.
\[
\hat{F}(\rho) = \hat{h}_1 + \hat{G}(\rho)
\]
where $\hat{h}_1$ is the one electron Hamiltonian containing the kinetic energy operator and the electron-nuclear interaction operator. $\hat{G}$ is the approximation of the two electron interaction which accounts for electron correlation, here accounted for as an average interaction. The nuclear-nuclear interaction is approximated to constant so not accounted for in the Fock operator. With a finite set of basis functions the Fock operator is approximated to the Fock matrix:
\[
...
where F is the Fock matrix and S is the overlap matrix which will reduce to the identity matrix if we have an orthonormal basis set. $\epsilon$ is a diagonal matrix of the orbital energies. C is the coefficient matrix of the occupied states, this is used to get the new  density matrix $\rho$ =  $C_{occ}$*$C_{occ}$.
