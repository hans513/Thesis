...
Dagger is a powerful algorithm that can trains a model that perform well in its the induced state.
It is a 

In our case, we want to train a model for each iteration. 
...
In each iteration, we will learn a Hatree-Fock with embedded parameter $\pi_i$ that best 
...
We have two sources of the training example. One from the expert's demonstration, the other one is the state which is induced by the previous learned policy $\pi_2$.
