...
Since Hartree-Fock is a numerical method and the most straightforward way to get accurate result may cause the computation time to increase exponentially with the number of atoms, the whole process can easily become excessively time-consuming or even turn out to be infeasible very quickly.
...
The time-invariant Schro\"{o}dinger equation is an eigenvalue problem; in general, it will have multiple solutions $(E_1, \Psi_1), (E_2,\Psi_2), \ldots$ of different energies.  Each solution is called a (molecular) orbital, and represents a time-invariant wave function which a particle (in our case an electron) can occupy.
...
A linear combination of the basis functions can be used to define the wave-function of the molecular system. However, since wave functions cannot represent classical uncertainty or particle counts, we move instead to density operators.  If $\Psi_1, \Psi_2, \ldots$ are wave functions, and if $\alpha_1, \alpha_2, \ldots$ are positive real numbers, then
$$\rho = \sum_i \alpha_i \Psi_i\Psi_i^*$$
is a density operator, representing a system in which (on average) there are $\alpha_1$ particles that occupy wave function $\Psi_1$,  $\alpha_2$ that occupy $\Psi_2$, and so forth.

We approximate the density operator by its representation in our finite basis: 
$$\rho \approx \sum_{ij} \rho_{ij}\psi_i\psi_j^*$$

%======TODOTODO
 Molecular orbitals are orbitals created when two or more elements interact and are usually described using a linear combination of atomic orbitals. Density matrix is a probability matrix of the molecular orbitals within the system and is given by
\[
\rho = \sum_{i,j}\sum_\sigma|\phi_{i,j}(\sigma)|^2
\]
Where $\phi_{i,j}$ is the $i,j$th molecular orbital which is a wave function describing the behavior of an electron in a molecule.
$\sigma$ is the spin coordinate of the electron. 
%==============

The Restricted Hartree-Fock (RHF) which is the most commonly used model of HF, assumes all the lowest-energy orbitals are fully occupied. Given the Pauli exclusion principle, each orbital will carry two electrons, one of each spin. The equation can thus be simplified as the following:

\[
\rho = 2\sum_{i,j}|\phi_{i,j}|^2
\]
...
In HF the time invariant Schr\"{o}dinger equation is approximated into the Roothaan equation which has the following form
\[
FC = SC\epsilon
\]
...
where $F$ is the Fock matrix which is described with the Fock operator and a finite set of basis functions,
\[
...
\]
This matrix will reduce to the identity  if we have an orthonormal basis set. $\epsilon$ is a diagonal matrix of the orbital energies. $C$ is the coefficient matrix of all the states. In RHF it can be used to get the density matrix $\rho$ =  $C_{occ}{C_{occ}}^*$ in which $C_{occ}$ is the coefficient matrix of all the occupied states.
...
\begin{algorithm}[htb]
 ...
 \KwResult{Density matrix which gives minimum energy}
	Set up the size of the basis set/ Set up number of basis functions\\%can perhaps delete this
	Calculate one electron Hamiltonian($h_1$) by input data  \\% can maybe delete this too, I think that is what Geoff wanted
