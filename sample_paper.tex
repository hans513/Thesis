...
The Schrödinger equation is the foundation equation of quantum mechanics, in which the solution can give us a complete description of a particle or a molecular systems. The Schrödinger equation provides a way to calculate the possible wave functions of a system and how they dynamically change in time. A wave function describes the quantum state of a system of one or more particles, and contains all the information about the system considered in isolation. A time invariant steady state form of the Schrodinger equation is shown below, which is called the time-independent Schrodinger equation.
 $H\Psi = E\Psi$ 
The H is the Hamiltonian which operates on the wave-function, $\psi$, to give the energy of the wave-function, E. Hartree-Fock (HF) gives us an approximation of a solution to the schrodinger equation. In HF the wave-function is described with a density matrix, which is a probability matrix of the mixed states within the system. The wave-fucntion contains more information regarding the particle or system however it is difficult to construct a wave function for a many body system but approximating it a density matrix will allow us to calculate the approximated schrodinger equation or Hartree-Fock.
...
The general form of the Hamiltonian can be partitioned into two components: the one-electron operator and the two-electron operator. The one electron operator is 2 dimensional since it only considers the activity of a electron on one atom, the two-electron is 4 dimensional since it considers the interactions between two electrons on two different atoms.
...
Get new density $\rho_1$ = C*C\\
Set the new density $\rho = \rho_1$\\
\textbf{termination}\\
