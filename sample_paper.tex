...
However, since wave functions cannot represent classical uncertainty, density matrices are the exactly and only way to solve the problem. Density matrix is a probability matrix of the molecular orbitals within the system and is given by
...
Where $\phi_i$ is the $i$th molecular orbital which is a wave function describing the behavior of an electron in a molecule.
...
$\epsilon$ is a diagonal matrix of the orbital energies. $C_{occ}$ is the coefficient matrix of the occupied states. In RHF it can be used to get the density matrix $\rho$ =  $C_{occ}{C_{occ}}^*$.
