...
% Hatree-Fock intro
In computational chemistry and physics, Hartree-Fock is a method used to get an approximation 
of electronic distribution of a molecule and the energy of the system. Hartree-Fock is an iterative process and can be quite tedious and time consuming for large molecules especially if you want to get an accurate description of the orbitals. Speeding up the Hatree-Fock would enable us to deal with larger molecules and perhaps use it to study biochemical reaction which usually has very large systems. In essence, speeding up the Hatree-Fock algorithm will save computational cost for solving quantum chemical problems. 

% Parameters
Matteus et al, \cite{Matteus}, proposed a way to embed the parameters.....


The parameters give us the flexibility to control the behaviour of the Hatree-Fock method. With this flexibility, we can try to let each of the Hatree-fock iteration make more progress. In particular, we'd to make single iteration of Hatree-fock with parameters get as much progress as two or even more iterations of the original Hatree-fock.

% How to do that? (Imitation learning)
This idea can be achieved by imitation learning which is an algorithm that learns from expert demonstrations in order to become capable of performing particular tasks. By imitation learning, we can make Hatree-fock with parameters learns from original Hatree-Fock, and finally make it capable to gain two iterations or more progress in one single iteration.

% Problem encountered   %%%%%
However, imitation learning may yield poor performance in theory and practice  since the learner's prediction and action affects future input observations and states during execution of the learned policy. It obvious violates the common i.i.d. assumption made in most statistical learning approaches. \cite{Ross}.

% The recipe (DAgger)
Ross et al,\cite{DAgger} proposed the dataset aggregation algorithm (DAgger) which learns a stationary deterministic policy guaranteed to perform well under its induced distribution of states is a remedy to the poor performance in imitation learning.


In this paper, 
we conduct an interdisciplinary research to introduce an innovative technique - imitation learning to speed up the Hartree-Fock process.  Ross (cite:Ross et al.[2011]) proposed the dataset aggregation algorithm (DAgger), which was proven to have stable performance and faster learning rate. \cite{DAggerCompare}
...
F = $KE + sum_A EN_A + [2*J - K]$,

where KE is the kinetic energy operator, $EN_A$ is the electron-nuclear interaction operator for atom A and J and K are components of the electron-electron interaction operator.


The electrons or orbitals are usually described with a combination of Gaussian functions, and the cost of the algorithm scales as N\textsuperscript{p} where: 

N = Number of basis functions (features) 

 = n Natoms  (n = basis functions/atom)  

p  = depending on the accuracy model between 3.5 and 8 
