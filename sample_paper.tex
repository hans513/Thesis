...
In computational chemistry and physics, Hartree-Fock is a method used to get the density matrix for a given molecule. The density matrix describes the molecule and the probability distribution of the location of the electrons. Hartree-Fock is an approximation for solving the Schrödinger equation where it assumes that the wave function can be approximated by a single Slater determinant. The electrons or orbitals are described with a combination of Gaussian functions.

The Hartree-Fock method is an iterative process and can be quite tedious and time consuming for large molecules especially if you want to use more Gaussian functions to get a more accurate description of the orbitals. Speeding up the Hatree-Fock would enable us to deal with larger molecules and perhaps use it to study biochemical reaction which usually has very large systems. In essence speeding up the Hatree-Fock algorithm will save computational cost for solving quantum chemical problems. 

In this paper, we conduct an interdisciplinary research to introduce an innovative technique - imitation learning to speed up the Hartree-Fock process. Imitation learning is an algorithm that learn from expert demonstrations in order to become capable of performing particular tasks. Ross (cite:Ross et al.[2011]) proposed the dataset aggregation algorithm (DAgger), which was proven to have stable performance and faster learning rate. (cite:Andreas Vlachos, An investigation of imitation learning algorithms for structured prediction) 
