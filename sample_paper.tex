...
Our goal is to have the Hartree-Fock with embedded parameters achieve the same goal as original Hartree-Fock with less iterations, say half or even fewer.
...
With the fast forwarded trajectories to serve as expert's demonstration, in each single iteration $i$ in Hartree-Fock, we will use DAgger to learn a Hatree-Fock with embedded parameter $\pi_{i+1}$ that best speed up that iteration.  
...
Say, after training, with the best set of parameters learned, we can map density matrix $d_0$ to $d_{2}'$
...
In the second iteration of Hatree-Fock, we'll have two DAgger iterations. In the first DAgger iteration, the only training objective is from expert's demonstration, we want to train a set of parameter that best matches density matrix $d_2$ to $d_4$.

However, in the second DAgger iteration, we'll sample the trajectories using $\pi_{i-1,j-1}$, thus will get the dataset $D_i$ = $d_2'$ to $d_4$. Then the dataset is added into D.
%We then aggregate dataset that is the state induced by iteration 1,
Next step is training a policy that best fit D.
Thus we not only want to fit the expert's demonstration $d_2$ to $d_4$, but also want to fit the output of previous iteration $d_{2}'$ to $d_4$. The result of this policy then is the best policy that mimic the expert's demonstration in Hartree-Fock iterations two.
...
In the Hartree-Fock iteration i,  we'll apply DAgger algorithm to train a policy that best fit that iteration.
Thus in Dagger iteration j, j<i, we'll first sample the trajectories using $\pi_{i-1,j-1}$. That is the result after training in HF iter i-1, Dagger j-1. We then add the sample into the aggregated dataset D.
We want to train a policy that best fit all the density matrices in D to the true training objective.
As a result
we'll fit the expert's demonstration $d_{n-2}$ to $d_n$, and also all the possible output of previous iteration $d_{n-2}'$,  $d_{n-2}''$, $\ldots$,  $d_{n-2}^{n-1}$  to the true training objective$d_n$.
...
% Goal
The goal of the model fitting is to get the 1000 values generated from the ML-LL model to agree  with those from the HL theory.
...
We split the whole dataset into two part evenly. The odd numbered data serve as training data, on the other hand the even numbered data serve as testing data. \\

In our dataset, every training example contains density  within 30 iterations. So
After training on training data, we'll get 15 set of parameters 

We then test these parameters on the testing data.
