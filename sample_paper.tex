...
% and enable us to solve Schr\"{o}dinger equation by Hartree-Fock method. \\  
...
Where $\phi_i$ is the molecular orbital dependent on $\sigma$, the spin coordinate. In Restricted Hartree-Fock (RHF) which is most commonly used, the assume that all the orbitals are fully occupied, each orbital will carry two electrons given the Pauli exclusion principle and then the equation is describes as following:
...
\]
where $\hat{h}_1$ is the one electron Hamiltonian containing the kinetic energy operator and the electron-nuclear interaction operator. $\hat{G}$ is the approximation of the two electron interaction which accounts for electron correlation. $\rho$ is the density matrix. The nuclear-nuclear interaction is approximated to constant so not accounted for in the Fock operator. With a finite set of basis functions the Fock operator is approximated to the Fock matrix:
\[
